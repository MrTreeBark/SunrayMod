...
INSTALL_PATH=/home/pi/sunray_install/firmware
DEV_PATH=/home/pi/Sunray/alfred/firmware
...
function build_sunray_compatible() {
    ...
    runuser -l pi -c "arduino --pref build.path=/tmp/firmware --verify --board \"STMicroelectronics:stm32:GenF1:pnum=GENERIC_F103VETX\" $DEV_PATH/rm18"
    if [ -f /tmp/firmware/rm18.ino.bin ]; then
      runuser -l pi -c "cp /tmp/firmware/rm18.ino.bin $DEV_PATH"
      # install    
      if [ -d "$INSTALL_PATH" ]; then
        runuser -l pi -c "cp $DEV_PATH/rm18.ino.bin $INSTALL_PATH"
      fi
      true
    else  
