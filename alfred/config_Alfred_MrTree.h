...
#define ALFRED true             // is it Alfred with bananaPi or other Pi and RM18/21?
...
#define ROBOT_CONTROL_CYCLE         25    // (ms) cycle time when mowing
#define ROBOT_IDLE_CYCLE            50    // (ms) cycletime when charging
#define DIRECT_DRIVE                1     // uncomment this for alfred direct drive without stacking pid controllers and compute pwm on questionable sensor inputs like odometrie (flight by wire)
...
#define ROTATION_RAMP_MIN           10    // (deg/s) minimum rotation speed
...
#define MOWPOWERMIN                 40.0  // (Watt) idle Power of Mowmotor or minimum load power of mowmotor, if under this load mower will have maximum speed
...
#define MOW_PWM_NORMAL              200   // (pwm, only used if USE_MOW_RPM_SET = false) pwm of mow motor for normal mowing
#define MOW_PWM_SLOW                225   // (pwm, only used if USE_MOW_RPM_SET = false) pwm of mow motor when during mowing a keepslow state is triggered. Should be higher or the same as MOW_PWM_NORMAL
#define MOW_PWM_RETRY               225   // (pwm, only used if USE_MOW_RPM_SET = false) pwm of mow motor when during mowing a retryslow state is triggered (after escape lawn). Should be higher or the same as MOW_PWM_SLOW
...
#define CHANGE_SPEED_SET            false  // if enabled and rpm or power triggers escape lawn or a keepslow state, mower will go slow with RETRYSLOWSPEED or KEEPSLOWSPEED with configured RETRYSLOWTIME or KEEPSLOWTIME. Also, in this states MOW_RPM/PWM for _SLOW/RETRY are used for the mowmotor set
#define MOW_POWERtr_STALL           95    // (%, only used if ESCAPE_LAWN_MODE = 1) if power of mowmotor exceeds e.g 90% of MOWPOWERMAX, escapelawn is triggered 
#define MOW_POWERtr_SLOW            85    // (%, only used if ESCAPE_LAWN_MODE = 1) if power of mowmotor exceeds e.g 70% of MOWPOWERMAX, keepslow is triggered
...
#define NO_GPS_OBSTACLE_DEADTIME    10000  // (ms) time of gps obstacle ignorance after escapeLawnOp 
                          //test
...
#define NO_GPS_SIGNAL_TIMEOUT       1000 // (ms) mower is allowed to run in an GPS invalid state for given time.... 
#define GPS_JUMP_WAIT               false  // if a GPS jump is detected, should the mower wait?
...
#define ROTATION_TIMEOUT            8000  // Timeout of rotation movement that triggers an obstacle with escapeReverse (goal for shorter trigger times)
#define ROTATION_TIME               3000  // Time the code expects to rotate without a high IMU yaw difference (yaw difference configuration to be added)
...
#define MOWER_RADIUS_BACK           0.20  // (m) setObstaclePosition relevant (measure chassis from gps antenna and add 5cm´s)
...
#define SHOULDROTATE_DELAY          1500  // internal state bool will trigger shouldrotate with this delay after rotation command in code startet
...
#define WHEEL_DIAMETER        195        // (250) wheel diameter (mm)                 
...
#define TICKS_PER_REVOLUTION  280       // odometry ticks per wheel revolution (RM18modifiedFW)
...
// motor speed control (PID coefficients) - these values are tuned for Ardumower motors, Alfred does not use it in the Modded Sunray software
// general information about PID controllers: https://wiki.ardumower.de/index.php?title=PID_control
#define MOTOR_PID_LP      0.01    // encoder low-pass filter (use for low encoder tickcount - use zero to disable)
...
#define MOW_FAULT_CURRENT 8.5         // mowing motor fault current (amps)
...
#define WIFI_SSID "****"            // choose WiFi network ID
#define WIFI_PASS "****"      // choose WiFi network password
...
#define MQTT_TOPIC_PREFIX  "Alfred"                 // the MQTT topic prefix for your robot 
#define MQTT_SERVER  "192.168.178.11"                 // your MQTT broker IP or hostname (e.g. "broker.mqtt-dashboard.com")
#define MQTT_PORT  1883
#define MQTT_USER "alfred"
#define MQTT_PASS "alfred"
...
#define BUMPER_TRIGGER_DELAY  100 // bumper must be active for (ms) to trigger (do only use if you have a weak bumper(springs)), Bumper has already a code iteration delay of some milliseconds
...
#define GO_HOME_VOLTAGE   24  // start going to dock below this voltage
// The battery will charge if both battery voltage is below that value and charging current is above that value.
#define BAT_FULL_VOLTAGE  31.0  // start mowing again at this voltage
...
#define BAT_SWITCH_OFF_VOLTAGE 22.0       //switch off mower if under this voltage
...
#define GPS_SPEED_DEADTIME     4000  // (ms) give mower time after starting moving to look at the stategroundspeed from ublox
#define GPS_NO_SPEED_TIME      1000  // (ms) time for GPS no speed trigger --> obstacle
#define GPS_SPEED_DELAY        1000  // (ms) how long gps speed needs to be too low to trigger gps obstacle 
...
#define DOCK_LINEAR_SPEED 0.15   // linear speed for docking //MrTree doesnt do anything
...
#define DOCK_UNDOCK_TRACKSLOW_DISTANCE 0.01 // set distance (m) from dock for trackslow (speed limit)
...
#define STANLEY_CONTROL_P_NORMAL  3.0   // 3.4 // 3.0 for path tracking control (angular gain) when mowing
#define STANLEY_CONTROL_K_NORMAL  1.0  // 2.3 // 1.0 for path tracking control (lateral gain) when mowing

#define STANLEY_FLOAT_P_NORMAL         1.0
#define STANLEY_FLOAT_K_NORMAL         1.0

#define STANLEY_CONTROL_P_SLOW    0.1   // 1 // 3.0 for path tracking control (angular gain) when docking tracking
#define STANLEY_CONTROL_K_SLOW    0.1  // 0.05 // 0.1 for path tracking control (lateral gain) when mowing or docking

#define STANLEY_FLOAT_P_SLOW           0.1
